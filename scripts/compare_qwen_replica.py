...
sys.path[:0]=[str(ROOT/'src'),str(OUT/'replica-source')]
...
def main():
    ...
    report={'environment':{'python':platform.python_version(),'torch':torch.__version__,'transformers':transformers.__version__,
        ...
        'replica_revision':'2af86848be75847ccb3553b0941cc51d6ef7e4e9','repeats':a.repeats,'cases':{},
    ...
    for wi,(name,data,questions,schema)in enumerate(workloads):
        ...
        modes=['qwen_json','qwen_independent_scoring','hf_parallel','our_optimized'];modes=modes[wi:]+modes[:wi]
        for mode in modes:
            e.clear_optimization_cache();plain.clear_optimization_cache();gc.collect();torch.cuda.empty_cache()
            def call():
            ...
            try:
                torch.cuda.synchronize();start=time.perf_counter();warm=call();torch.cuda.synchronize();warm_ms=(time.perf_counter()-start)*1000
                ...
                m={'status':'ok','warmup_ms':warm_ms,'median_ms':statistics.median(x['wall_ms']for x in rows),
        ...
        save()
    e.clear_optimization_cache();print('COMPLETE',OUT/'comparison.json',flush=True)
